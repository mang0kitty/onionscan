...
for json_file in file_list:
    with open(json_file, "rb") as fd:
        ...
        if scan_result['sshKey']:
            print("%s => %s" %
                  (scan_result['hiddenService'], scan_result['sshKey']))

            if scan_result['sshKey'] in key_to_hosts:
                key_to_hosts[scan_result['sshKey']].append(
                    scan_result['hiddenService'])
            else:
                key_to_hosts[scan_result['sshKey']] = [
                    scan_result['hiddenService']]
for ssh_key in key_to_hosts:
    if len(key_to_hosts[ssh_key]) > 1:
        print("[!] SSH Key %s is used in multiple hidden services." % ssh_key)
    ...
    if shodan_result['total'] > 0:
        for hit in shodan_result['matches']:
            print("[!] Hit for %s on %s for hidden services %s" %
                  (ssh_key, hit['ip_str'], ",".join(key_to_hosts[ssh_key])))
